...
public class LoadMaster extends javax.swing.JFrame {
    ...
    public static boolean batchToProcess;
    ...
    public LoadMaster() {
        ...
        batchToProcess = Starter.props.getPropertyAsBoolean("unprocessed.batch", 
                "false");
    }
    ...
    private void createSystemTasks() {
        ...
        systemTasks.add(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                CustomerEntry dlg = new CustomerEntry();
                mainDesktop.add(dlg);
                dlg.pack();
                dlg.setVisible(true);
            }
        ...
        systemTasks.add(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                BrokerEntry dlg = new BrokerEntry();
                mainDesktop.add(dlg);
                dlg.pack();
                dlg.setVisible(true);
            }
    ...
    private void createAccountingTasks() {
        ...
        accountingTasks.add(new JSeparator());

        accountingTasks.add(new AbstractAction() {
            {
                putValue(Action.NAME, "General Ledger Viewer...");
                putValue(Action.SHORT_DESCRIPTION, "Displays the General Ledger "
                        + "Viewer.");
                putValue(Action.SMALL_ICON, new javax.swing.ImageIcon(getClass()
                        .getResource("/com/pekinsoft/loadmaster/res/Accounting.png")));
            }
    ...
    private void doClose() {
        if (batchToProcess) {
            String msg = "There are unprocessed transactions.\n\n"
                    + "Do you want to exit Load Master™ anyway?\n\n"
                    + "You can process your batched transactions\n"
                    + "the next time you run Load Master™ if you\n"
                    + "exit now.";
            int response = MessageBox.askQuestion(msg, "Confirm Close", false);

            if (response == MessageBox.YES_OPTION) {
                Starter.props.setPropertyAsBoolean("unprocessed.batch", true);
                Starter.props.setPropertyAsInt("windows.main.state",
                        getExtendedState());

                Starter.props.setPropertyAsInt("windows.main.tb",
                        tbSplit.getDividerLocation());
                Starter.props.setPropertyAsInt("windows.main.lr",
                        lrSplit.getDividerLocation());

                if (getExtendedState() != JFrame.MAXIMIZED_BOTH) {
                    Starter.props.setPropertyAsInt("windows.main.top",
                            getLocation().y);
                    Starter.props.setPropertyAsInt("windows.main.left",
                            getLocation().x);
                    Starter.props.setPropertyAsInt("windows.main.height",
                            getSize().height);
                    Starter.props.setPropertyAsInt("windows.main.width",
                            getSize().width);
                }
                Starter.exit(SysExits.EX_OPCANCEL);
            }
        } else
    ...
    private void doCashAdvance() {
        CashAdvanceDialog dlg = new CashAdvanceDialog(this, false);
        dlg.pack();
...
//        loadProgress.setValue(Starter.props.getPropertyAsInt("load.stop", "0"));
    ...
    private void doShowGeneralLedger() {
        GeneralLedgerViewer window = new GeneralLedgerViewer();
