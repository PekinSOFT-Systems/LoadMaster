...
/**
 * `ArgumentParser` is a library for parsing command line arguments in an easy,
 * intelligible and efficient manner. This parser breaks down the difference
 * between switches and targets. Switches are typically single letter options
 * that are prefixed with a single dash (-) or, word or multi-word options that
 * are prefixed with a double dash (--). Targets are the parameters to the
 * switches.
 *
 * For example, if your program requires a flag for debugging, you would provide
 * a single `boolean` switch that takes no parameters, such as `-d` or
 * `--debug`. Your program would then simply check if that switch was present,
 * which would equate to `debugging = true`, or not, which would equate to
 * `debugging = false`.
 *
 * Alternatively, if the debugging flag is present, you may want to set a
 * debugging level for verbosity of output. In that use case, your program could
 * check for the existence of the debugging flag (above) and, if it exists,
 * check for the debugging level, which could be `--debugging-level 2` for a
 * medium verbosity output. In this instance, the switch is `--debugging-level`
 * and the target is 2.
 *
 * This works for any type of switch and target. You can even have targets with
 * no switches or multiple targets for each switch. If you need targets without
 * a switch, those targets should be listed first on the command line, as this
 * library interprets all targets following a switch as belonging to that
 * switch, up to the next switch on the command line.
 *
 * Also, you can use this library to populate fields in your custom classes,
 * provided you name your fields the same as the command line switch that
 * gathers the data for those fields.
 *
...
public class ArgumentParser {
    ...
    /**
     * Creates an instance of the `ArgumentParser` class to parse command line
     * arguments into something useful.
     *
    ...
    /**
     * Checks to see if the given switch is present in the command line
     * arguments.
     *
     * @param switchName The command line switch to check for.
     * @return {@code true} if the switch is present; {@code falcse} otherwise.
     */
     ...
     * present.
     * @return The value, if the switch is present; the supplied default value
     * if the switch was not found.
     */
     ...
     * @param defaultValue A default value in the event the switch is not
     * present.
     * @return {@code java.lang.Long} value, if the switch is present; the
     * supplied default value if the switch is not found.
     * @throws NumberFormatException if the string does not contain a parsable
     * long.
     */
     ...
     * @param defaultValue A default value in the event the switch is not
     * present.
     * @return {@code java.lang.Double} value, if the switch is present; the
     * supplied default value if the switch is not found.
     * @throws NumberFormatException if the string does not contain a parsable
     * double.
     */
     ...
     * @return An array of {@code java.lang.String}s containing all of the
     * values associated with the given switch. If the switch is not found, an
     * array of length zero (0) will be returned.
     */
     ...
     * @throws RuntimeException in the event the POJO cannot be created.
     */
